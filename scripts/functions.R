...
behaviorplot <- 
  function(emmeanDataframe, Phase, Breathing_Condition, ylabel){
    ggplot(emmeanDataframe, aes(x=Phase, y=emmean, color=Breathing_Condition)) +
      geom_point(size = 6, position = pd) + # was 3
      geom_line(aes(linetype = Breathing_Condition, group = Breathing_Condition),size = 2, position = pd)+
...
stateplot <-function(data, emmean_dataframe, var, title){
  ggplot()+ 
    geom_flat_violin(data= data, aes(x= Phase, y= .data[[var]], fill=Breathing_Condition),position = position_nudge(x =.3, y = 0), adjust = 1.5, alpha = .5, colour = NA)+ # flat violin distribution, .3 points to the right. alpha=.5 so see-through
...
cohens_d_paradigm_between <- function(dataframe, var, testmoment, group1, group2){
  temp = dataframe[dataframe$Breathing_Condition == group1 & dataframe$Phase == testmoment | dataframe$Breathing_Condition == group2 & dataframe$Phase == testmoment ,] # Create a dataframe with only relevant data
  formula <- as.formula(paste0(var, ' ~ Breathing_Condition')) # Create formula of interest with as.formula()
